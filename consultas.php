...
$QueryClientsPlanilla="SELECT cliente.idCliente, cliente.idPersona, concat(rtrim(person.name),' ',rtrim(person.lastname), '#',person.cedula) as nombreCompleto  from cliente inner join person on cliente.idPersona= person.id  order by person.name";
